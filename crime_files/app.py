...
init_district_name = "District with highest Crime : {}".format(filters.get_district_crime_high())
# initial count of total crimes
init_crime_count = "Total Offences : {}".format(filters.get_total_cases())
# initial arrest percentage
init_arrest_percentage = "Arrest Done : {:.2f}%".format(filters.get_percentage_arrest())
# initial domestic violence percentage
init_domestic_percentage = "Domestic Abuse : {:.2f}%".format(filters.get_percentage_domestic())
...
story_buttons = html.Div([
    dbc.Button("Where we are?", id="btn-str-1", n_clicks=0, color="secondary", className="me-1 four columns",),
    dbc.Button("Effective Chicago PD?", id="btn-str-2", n_clicks=0, color="secondary", className="me-1 five columns",),
    dbc.Button("Criminals sleep at night...", id="btn-str-3", n_clicks=0, color="secondary", className="me-1 six columns",),
    dbc.Button("Criminals on a holiday!!", id="btn-str-4", n_clicks=0, color="secondary", className="me-1 five columns",),
...
holiday_crime_block = html.Div([
    html.Div(
        className = "row", children = [
            ...
            html.Div(className="Element", children = [
                ...
                figures.dropdown_filter_fig("crime_type_1",filters, False, "Arson")
            ], style=dict(width="33.3%",padding="15px")),
            html.Div(className="Element", id="filter_district", children = [
                html.Label("Select Districts"),
                figures.dropdown_filter_fig("districts_1", filters, False, "Central")
    ...
    html.Div(
        className = "row", children = [
            ...
            html.Div(className="Element", children = [
                html.Label("Select Crime Type"),
                figures.dropdown_filter_fig("crime_type_2",filters, False, "Assault")
            ], style=dict(width="33.3%",padding="15px")),
            html.Div(className="Element", id="filter_district", children = [
                html.Label("Select Districts"),
                figures.dropdown_filter_fig("districts_2", filters, False, "Central")
...
weekday_crime_block = html.Div([
    html.Div(
        className = "row", children = [
            ...
            html.Div(className="Element", children = [
                html.Label("Select Crime Type"),
                figures.dropdown_filter_fig("crime_type_1",filters, False, "Assault")
            ], style=dict(width="33.3%",padding="15px")),
            html.Div(className="Element", children = [
                html.Label("Select Month"),
                figures.dropdown_filter_fig("months_1", filters, False, "January")
    ...
    html.Div(
        className = "row", children = [
            ...
            html.Div(className="Element", children = [
                html.Label("Select Crime Type"),
                figures.dropdown_filter_fig("crime_type_2",filters, False, "Arson")
            ], style=dict(width="33.3%",padding="15px")),
            html.Div(className="Element", children = [
                html.Label("Select Month"),
...
daytime_crime_block = html.Div([
    ...
    html.Div(
        className = "row", children = [
            ...
            html.Div(className="Element", children = [
                html.Label("Select Crime Type"),
                figures.dropdown_filter_fig("crime_type_2", filters, False, "Battery")
            ], style=dict(width="20%",padding="15px")),
            html.Div(className="Element", id="filter_district", children = [
                html.Label("Select Districts"),
                figures.dropdown_filter_fig("districts_2",filters, False, "Central")
                ...
                html.Div(
                    [
                        ...
                        html.Div(children=[
                                ...
                                html.Div(children=[figures.pydeck_elevation_fig(filters.geo_plot_filter())], id="map_graph"),
                            ],
...
def filter_count_display(types, years, districts, months, map_view):
    ...
    district_name = "District with highest Crime : {}".format(filters.get_district_crime_high())
    # arrest percentage
    arrest_percentage = "Arrest Done : {:.2f}%".format(filters.get_percentage_arrest())
    # domestic percentage
    domestic_percentage = "Domestic Abuse : {:.2f}%".format(filters.get_percentage_domestic())

    # pydeck maps data and figures
    pydeck_data_frame = filters.geo_plot_filter(new_data_frame)
    pydeck_fig = figures.pydeck_elevation_fig(pydeck_data_frame) if map_view == "elevate" else figures.pydeck_scatter_fig(pydeck_data_frame)
...
@app.callback(
    ...
    Input("id_slider_years_2", "value"),
     ...
     Input("id_dropdown_districts_2", "value"),
     Input("id_dropdown_months_1", "value"),
     Input("id_dropdown_months_2", "value")]
)
def effective_pd_filter(years1, years2, districts1, districts2, months1, months2):
    if years1 is None:
        years1 = []
    if years2 is None:
        years2 = []
...
def holiday_crime_filter(years1, years2, types1, types2, districts1, districts2):
    if years1 is None:
        years1 = []
    if years2 is None:
        years2 = []
    ...
    districts1 = [districts1] if districts1 is not None else []
    districts2 = [districts2] if districts2 is not None else []
    new_data_frame1 = filters.create_selection(years=years1, types=types1, districts=districts1, months=[])
    new_data_frame2 = filters.create_selection(years=years2, types=types2, districts=districts2, months=[])
    data_frame_1, mean1 = filters.holiday_crime_filter(new_data_frame1)
    data_frame_2, mean2 = filters.holiday_crime_filter(new_data_frame2)
    return (figures.holiday_crime_fig(data_frame_1,mean1),figures.holiday_crime_fig(data_frame_2,mean2))
...
def crime_during_weekday_filter(years1, years2, types1, types2, months1, months2):
    ...
    months1 = [months1] if months1 is not None else []
    months2 = [months2] if months2 is not None else []
    new_data_frame1 = filters.create_selection(years=years1, types=types1, districts=[], months=months1)
    new_data_frame2 = filters.create_selection(years=years2, types=types2, districts=[], months=months2)
...
def crime_daytime_filter(years1, years2, types1, types2, districts1, districts2, months1, months2):
    if years1 is None:
        years1 = []
    if years2 is None:
        years2 = []
    ...
    districts1 = [districts1] if districts1 is not None else []
    districts2 = [districts2] if districts2 is not None else []
    new_data_frame1 = filters.create_selection(years=years1, types=types1, districts=districts1, months=months1)
    new_data_frame2 = filters.create_selection(years=years2, types=types2, districts=districts2, months=months2)
    data_frame_1, mean1 = filters.daytime_crime_filter(new_data_frame1)
    data_frame_2, mean2 = filters.daytime_crime_filter(new_data_frame2)
    return (figures.daytime_crime_fig(data_frame_1,mean1),figures.daytime_crime_fig(data_frame_2,mean2))
...
def crime_abuse_filter(years1, years2, months1, months2):
    ...
    months1 = [months1] if months1 is not None else []
    months2 = [months2] if months2 is not None else []
    new_data_frame1 = filters.create_selection(years=years1, types=[], districts=[], months=months1)
    new_data_frame2 = filters.create_selection(years=years2, types=[], districts=[], months=months2)
