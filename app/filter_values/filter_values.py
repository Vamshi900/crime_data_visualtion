...
class LoadFilterValues:
    ...
    def create_selection(self, years=[2001, 2022], types=[], districts=[], months=[]):
        ...
        if len(years) > 0:
            temp_data_frame = temp_data_frame.loc[temp_data_frame["Year"].isin(
                year for year in range(years[0], years[1]+1))]
        if len(districts) > 0:
            temp_data_frame = temp_data_frame.loc[temp_data_frame["District"].isin(
                districts)]
        if len(months) > 0:
            temp_data_frame = temp_data_frame.loc[temp_data_frame["Month"].isin(
                months)]
        if len(types) > 0:
            temp_data_frame = temp_data_frame.loc[temp_data_frame["Primary Type"].isin(
                type.upper() for type in types)]
